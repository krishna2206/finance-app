/**
 * Grille tarifaire officielle indicative MVola (Madagascar)
 * Calcul automatique des frais d'envoi et de retrait (Cash Point)
 */
...
export function calculateMVolaFees(amount: number): FeeBreakdown {
  ...
  if (amount <= 1000) {
    ...
    transferFee = Math.min(5000, Math.round(amount * 0.0035));
  }
  ...
  if (amount <= 1000) {
    ...
    withdrawalFee = Math.round(amount * 0.012);
  }
